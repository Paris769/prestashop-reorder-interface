...
def _norm_txt(s: str) -> str:
    ...
    s = re.sub(r"\b(cm|mm|ml|lt|pz|conf|cf|kg|gr|\xf8|diam|x)\b", " ", s)
...
def load_order_excel(file) -> pd.DataFrame:
    ...
    out = pd.DataFrame()
    if c_code is not None: out["order_itemcode"] = df[c_code].astype(str).str.strip()
    if c_desc is not None: out["order_desc"]     = df[c_desc].astype(str).str.strip()
...
def load_order_pdf(file) -> pd.DataFrame:
    ...
    for c in df.columns:
        if c.startswith("col"):
            ...
            if alnum>20: df["order_itemcode"] = df["order_itemcode"].mask(df["order_itemcode"]=="", df[c])
            if numeric>20:
            ...
            if longtxt>20: df["order_desc"] = df["order_desc"].mask(df["order_desc"]=="", df[c])

    df["order_desc_norm"] = df["order_desc"].map(_norm_txt)
    return df[["order_itemcode","order_desc","order_desc_norm","order_qty"]]


def build_customer_stats(df_sales: pd.DataFrame, customer_id: Optional[str]) -> Tuple[dict, dict]:
    """Ritorna (freq_norm, recency_norm) sul sottoinsieme del cliente."""
    df = df_sales.copy()
    ren = {}
    for c in df.columns:
        cl = str(c).lower()
        if cl in ["cardcode","cliente","codice cliente/fornitore"]: ren[c]="customer_id"
        if cl in ["itemcode","codice articolo","articolo","sku","prodotto"]: ren[c]="product_id"
        if cl in ["itemname","descrizione articolo","descrizione","name","product"]: ren[c]="name"
        if cl in ["quantity","qta","qtasped","quantità","qty","pezzi"]: ren[c]="qty"
        if cl in ["docdate","data","date"]: ren[c]="order_date"
        if cl in ["ddt","docnum","numero ddt","order_id"]: ren[c]="order_id"
...
def match_order_to_catalog(
    ...
    """Fuzzy matching + probabilità d'acquisto storico."""
    if fuzz is None:
    ...
    for _, r in order_df.iterrows():
        ...
        if ocode and ocode in set(cat["product_id"]):
            row = cat[cat["product_id"]==ocode].iloc[0]
            results.append({
                "order_itemcode": ocode, "order_desc": oname, "order_qty": qty,
                "matched_itemcode": ocode, "matched_name": row["name"],
                "probability": 1.0, "method": "code", "status": "OK", "candidates": None
            })
            continue
        ...
        if oname_norm:
            ...
            if len(exact)>0:
                ...
                results.append({
                    "order_itemcode": ocode, "order_desc": oname, "order_qty": qty,
                    "matched_itemcode": row["product_id"], "matched_name": row["name"],
                    "probability": 0.90, "method": "desc_exact", "status": "OK", "candidates": None
                })
                continue
        ...
        if not oname_norm:
            results.append({
                "order_itemcode": ocode, "order_desc": oname, "order_qty": qty,
                "matched_itemcode": None, "matched_name": None,
                "probability": 0.0, "method": "no_name", "status": "NON TROVATO", "candidates": None
            })
            continue
        ...
        candidates = [{"product_id":pid,"name":nm,"prob":round(float(p),3)} for (pid,nm,p,_,_) in sims[:topk]]
        results.append({
            "order_itemcode": ocode, "order_desc": oname, "order_qty": qty,
            "matched_itemcode": pid_best if p_best>=review_thresh else None,
            "matched_name": name_best if p_best>=review_thresh else None,
            "probability": round(float(p_best),3), "method":"desc_fuzzy",
            "status": status, "candidates": json.dumps(candidates, ensure_ascii=False)
        })
    return pd.DataFrame(results)


def export_sap_excel(header: dict, lines: pd.DataFrame) -> bytes:
    """Esporta file Excel con fogli ORDR/RDR1 (compatibile DTW SAP B1)."""
